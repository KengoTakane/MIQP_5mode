...
# 離散時間区分アフィンモデルと非線形微分方程式の時間スケールを合わせる必要あり．
...
for j in range(time):
    if j <= tf:
        ...
        constr_mpc += [q_1[:,N] >= q1_min, Ta <= Ta_max, Ta >= Ta_min, Rh <= Rh_max, Rh >= Rh_min]
...
for j in range(time):
    if j <= tf:
        ...
        for k in range(j,j+N):
            ...
            t = t+1
        cost_mpc += w3*cp.square(q_1[:,N] - desire*q_1star_B[j])
        if j <= tm:
...
ax2.plot(range(tm), q_2star[0,0:tm], label="$q_{2}(k)$(MPC of A)")
ax2.plot(range(tm), q_2star_B[0,0:tm], label="$q_{2}(k)$(MPC of B)")
